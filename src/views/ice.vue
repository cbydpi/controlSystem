  ...
  <div class="ice_screen">
    <div class="ice_screen_main">
      ...
      <div style="text-align: center;">
        <span style="font-size: 24px;position: absolute;top: -15px;left: 20px;">冰屏</span>
        <i class="cbiconfont voice" @click="muted" :class="flag ? 'cbicon-voiceon': 'cbicon-voiceoff'"></i>
        <el-button type="primary" class="button_1" @click="play">播放/暂停</el-button><br>
        <el-button type="warning" class='button_2' @click="prev">上一个</el-button><br>
        <el-button type="warning" class='next_step' @click="next">下一个</el-button>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    wsMessage: function (evt) {
      if (JSON.parse(evt.data).code === undefined) {
        this.onlineStatus = JSON.parse(evt.data)[31].online
        if (JSON.parse(evt.data)[31].status === 0 && this.onlineStatus === 1) {
          this.currentMode = '待机模式'
        } else if (JSON.parse(evt.data)[31].status === 1 && this.onlineStatus === 1) {
